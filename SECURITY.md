...
- Upload handlers must validate extension and MIME/content, sanitize filenames, enforce `MAX_UPLOAD_BYTES`, store immutable raw bytes outside the web root, and scan where the deployment platform supports it. Upload is intentionally not implemented in this bootstrap.
